...
3. Declare persisted public properties through `apply_component.properties`; the generated shell owns `api.property`, `api.serializable`, `api.displayName`, and `api.editorType`. Keep transient state private and do not edit the generated decorator region.
4. Discover and describe the `code` Toolset's `create_component`, `read_component`, `apply_component`, `write_component_body`, and `lint` tools. Use `preset.attach_user_component` when the requested target requires the exposed preset-root attachment path.
5. Generate User Component lifecycle and custom-event code only from its exact component declaration or read-only template. Do not infer APIs from old Blockly Event or Instruction names.
