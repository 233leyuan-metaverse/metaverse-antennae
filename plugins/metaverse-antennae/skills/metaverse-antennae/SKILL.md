...
4. Skill Resources and Catalog discovery are optional reference material, not execution routes. Consult them only when the user explicitly asks for domain guidance or when Authoring declaration discovery has already proved that additional terminology is required; after consulting them, return to Authoring discovery rather than executing through Catalog.
...
6. For gameplay or multi-step work, keep each independently owned action separate unless the discovered Authoring workflow safely composes them. Treat each write Operation's own semantic readback as its completion boundary.
