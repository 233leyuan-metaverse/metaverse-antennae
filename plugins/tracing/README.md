...
`dist/index.mjs` is a release output. Do not commit it. The publish pipeline builds it with `scripts/build-tracing-hook.ps1` and injects the write-only ingest token from `.data/tracing-ingest.json`.
