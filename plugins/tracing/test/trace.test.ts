...
function stageFixtures(): string {
  ...
  fs.cpSync(fixturesRoot, path.join(dir, "sessions"), {
    recursive: true,
    filter: (source) => !source.endsWith(".langfuse"),
  });
...
describe("antennae MCP upload gate", () => {
  const gatedConfig: Config = {
    ...baseConfig,
    require_mcp_servers: ["antennae_sideapi", "metaverse-antennae"],
  };

  it("skips a session that never called Antennae", async () => {
    const dir = stageFixtures();
    await convertRollout(path.join(dir, "rollout-two-turns-main.jsonl"), { config: gatedConfig });
    expect(exporter.getFinishedSpans()).toHaveLength(0);
  });

  it("skips a session that only used other MCP servers", async () => {
    const dir = stageFixtures();
    await convertRollout(path.join(dir, "rollout-tools-main.jsonl"), { config: gatedConfig });
    expect(exporter.getFinishedSpans()).toHaveLength(0);
  });

  it("uploads from the first Antennae turn onward as a Turn → LLM → Tool tree", async () => {
    const dir = stageFixtures();
    await convertRollout(path.join(dir, "rollout-antennae-gated.jsonl"), { config: gatedConfig });
    ...
    const roots = spans
      ...
      .sort((a, b) => startMs(a) - startMs(b));
    expect(roots).toHaveLength(2);
    expect(attr(roots[0], "langfuse.observation.input")).toContain("Create a weapon");
    expect(attr(roots[1], "langfuse.observation.input")).toContain("looks good");
    expect(spans.some((s) => attr(s, "langfuse.observation.input")?.includes("hi there"))).toBe(
      false,
    );

    const antennae = spans.find((s) => s.name === "antennae_sideapi.system_status");
    expect(antennae, "expected the Antennae MCP tool observation").toBeDefined();
    expect(obsType(antennae!)).toBe("tool");
    expect(parentId(antennae!)).toBe(
      spans
        .find((s) => s.name === "LLM" && parentId(s) === roots[0].spanContext().spanId)
        ?.spanContext().spanId,
    );
