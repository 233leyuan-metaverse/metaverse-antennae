...
import { DEFAULT_BASE_URL, DEFAULT_INGEST_TOKEN, DEFAULT_REQUIRE_MCP_SERVERS } from "./defaults.js";
...
export const ConfigSchema = z.object({
  // TRACE_TO_LANGFUSE; defaults to true (opt out with "false")
  enabled: z.boolean(),
  // LANGFUSE_CODEX_PUBLIC_KEY | LANGFUSE_PUBLIC_KEY
  public_key: z.string().optional(),
  // LANGFUSE_CODEX_SECRET_KEY | LANGFUSE_SECRET_KEY
  secret_key: z.string().optional(),
  // LANGFUSE_CODEX_INGEST_TOKEN — write-only token for the Codex ingest route
  ingest_token: z.string().optional(),
  // LANGFUSE_CODEX_REQUIRE_MCP_SERVERS — gate uploads on these MCP server names
  require_mcp_servers: z.array(z.string()),
...
> = {
  ...
  ingest_token: DEFAULT_INGEST_TOKEN,
  require_mcp_servers: [...DEFAULT_REQUIRE_MCP_SERVERS],
  max_chars: 20_000,
  debug: false,
  fail_on_error: false,
};
  ...
  try {
    ...
    return PartialConfigSchema.parse(
      stripUndefined({
        ...
        require_mcp_servers:
          raw.require_mcp_servers != null ? parseStringList(raw.require_mcp_servers) : undefined,
...
function readEnvConfig(env: Record<string, string | undefined>): Partial<Config> {
  return PartialConfigSchema.parse(
    stripUndefined({
      ...
      ingest_token: env.LANGFUSE_CODEX_INGEST_TOKEN,
      require_mcp_servers: parseStringList(env.LANGFUSE_CODEX_REQUIRE_MCP_SERVERS),
