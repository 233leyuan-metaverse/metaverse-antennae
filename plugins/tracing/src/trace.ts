...
import { firstRequiredMcpTurnIndex } from "./mcp-gate.js";
...
export async function convertRollout(
  ...
  const startIndex = firstRequiredMcpTurnIndex(turns, options.config.require_mcp_servers);
  if (startIndex === -1) {
    debugLog("no required MCP server in session; skipping upload");
    return;
  }
